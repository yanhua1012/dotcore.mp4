...
public sealed class ConsoleSmokeTests
{
    ...
    [Theory]
    [InlineData("progressive", "h264", "async", "I/O: async")]
    [InlineData("faststart", "h264", "async", "I/O: async")]
    [InlineData("fragmented", "h264", "async", "I/O: async")]
    [InlineData("progressive", "h265", "async", "I/O: async")]
    [InlineData("faststart", "h265", "async", "I/O: async")]
    [InlineData("fragmented", "h265", "async", "I/O: async")]
    public void ConsoleAsyncModeWritesReopensAndPrintsAsyncMarker(
        string requestedMode,
        string requestedCodec,
        string ioMode,
        string expectedIoLine)
    {
        var root = FindRepositoryRoot();
        var project = Path.Combine(root, "samples", "DotCore.Mp4.Console", "DotCore.Mp4.Console.csproj");
        var output = Path.Combine(Path.GetTempPath(), "dotcore-console-async-" + Guid.NewGuid().ToString("N") + ".mp4");
        try
        {
            var arguments = "run --project " + Quote(project) + " --no-build -- " + Quote(output) +
                            " " + requestedMode + " " + requestedCodec + " " + ioMode;
            var startInfo = new ProcessStartInfo("dotnet", arguments)
            {
                UseShellExecute = false,
                RedirectStandardOutput = true,
            ...
            process.WaitForExit();
            Assert.True(process.ExitCode == 0, stderr);
            Assert.True(File.Exists(output), "Console async did not create its output file.");
            Assert.Contains(expectedIoLine, stdout, StringComparison.Ordinal);
            var outputLines = stdout.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            Assert.Equal(3, outputLines.Where(line => line.StartsWith("Video NAL:", StringComparison.Ordinal)).Count());
            Assert.Equal(4, outputLines.Where(line => line.StartsWith("AAC sample:", StringComparison.Ordinal)).Count());

            using var stream = File.OpenRead(output);
            using var reader = new Mp4Reader(stream);
            Assert.Equal(requestedCodec == "h265" ? VideoCodec.H265 : VideoCodec.H264, reader.VideoConfiguration!.Codec);
            Assert.Equal(new[] { true, false, true }, reader.ReadVideoNalUnits().Select(sample => sample.IsKeyFrame));
            Assert.Equal(4, reader.ReadAudioSamples().Count());
        }
        finally
    ...
    [Fact]
    public void ConsoleDefaultAndExplicitSyncPrintsSyncMarker()
    {
        var root = FindRepositoryRoot();
        var project = Path.Combine(root, "samples", "DotCore.Mp4.Console", "DotCore.Mp4.Console.csproj");
        foreach (var extra in new[] { "", " sync" })
        {
            var output = Path.Combine(Path.GetTempPath(), "dotcore-console-sync-" + Guid.NewGuid().ToString("N") + ".mp4");
            try
            {
                var arguments = "run --project " + Quote(project) + " --no-build -- " + Quote(output) + " progressive h264" + extra;
                var startInfo = new ProcessStartInfo("dotnet", arguments)
                {
                    UseShellExecute = false,
                    RedirectStandardOutput = true,
                ...
                var stdout = process.StandardOutput.ReadToEnd();
                process.WaitForExit();
                Assert.Contains("I/O: sync", stdout, StringComparison.Ordinal);
                Assert.Contains("Mode: progressive", stdout, StringComparison.Ordinal);
            }
            finally
            {
                if (File.Exists(output)) File.Delete(output);
    ...
    [Fact]
    public void ConsoleRejectsUnknownIoModeWithoutCreatingSuccessOutput()
    {
        var root = FindRepositoryRoot();
        var project = Path.Combine(root, "samples", "DotCore.Mp4.Console", "DotCore.Mp4.Console.csproj");
        var output = Path.Combine(Path.GetTempPath(), "dotcore-console-badio-" + Guid.NewGuid().ToString("N") + ".mp4");
        var startInfo = new ProcessStartInfo(
            "dotnet",
            "run --project " + Quote(project) + " --no-build -- " + Quote(output) + " progressive h264 badio")
        {
            UseShellExecute = false,
            RedirectStandardOutput = true,
            RedirectStandardError = true,
